...
while True:
    ...
    for i in range(edges-1):
        ...
        PLister()
        ContLister()
    # Place new edges based on the configuration of TokenList, and refresh the lists accordingly

    print(EdgeList)
    # Print EdgeList for accurate confirmation

    NodePlot=[]
    for i in range(nodes):
        r=(1/(TokenList[i]))**(1/nodes)
    ...
    # Therefore, we use the PList element; the distance from the center should somewhat be disproportional of the element.
    # The angle in which the element is located from the center is decided randomly (random.uniform).
    # The NodePlot and plotting section should be moved to the back so that PList works.
    ...
    print('\n\n=========================================')
    RestartPref=input('Restart the program? (y/n): ')
    if RestartPref=='y':
        del RestartPref
    elif RestartPref=='n':
        break
